...
public sealed class SectionListLoaderTests
{
    ...
    [Fact]
    public async Task LoadAsync_non_English_skips_retry_when_all_sections_already_cataloged()
    {
        var sections = new SortedDictionary<string, BiblePublicationSection>(StringComparer.OrdinalIgnoreCase)
        {
            ["1"] = new BiblePublicationSection { Id = 10, SectionCode = "1", Name = "Genesis" },
        };
        var media = new NonEnglishSectionMedia(sections, expectedCount: 1);
        var sut = new SectionListLoader(TestLogging.CreateLogger(), media, internetChecker: null);

        var (items, map) = await sut.LoadAsync("S", "nwtsty", selectedSectionCode: null);

        Assert.Single(items);
        Assert.Equal(0, media.FetchWithProgressCalls);
    }

    [Fact]
    public async Task LoadAsync_non_English_throws_when_offline_and_catalog_incomplete()
    {
        var sections = new SortedDictionary<string, BiblePublicationSection>(StringComparer.OrdinalIgnoreCase)
        {
            ["1"] = new BiblePublicationSection { Id = 0, SectionCode = "1", Name = "Placeholder" },
        };
        var media = new NonEnglishSectionMedia(sections, expectedCount: 2);
        var sut = new SectionListLoader(
            TestLogging.CreateLogger(),
            media,
            internetChecker: new OfflineConnectivityChecker());

        await Assert.ThrowsAsync<HttpRequestException>(() =>
            sut.LoadAsync("S", "nwtsty", selectedSectionCode: null));
    }

    [Fact]
    public async Task LoadAsync_reports_progress_to_reporter()
    {
        var sections = new SortedDictionary<string, BiblePublicationSection>(StringComparer.OrdinalIgnoreCase)
        {
            ["1"] = new BiblePublicationSection { Id = 5, SectionCode = "1", Name = "Genesis" },
        };
        var reporter = new RecordingFetchProgress();
        var sut = new SectionListLoader(
            TestLogging.CreateLogger(),
            new StubMedia { SectionsResult = sections },
            internetChecker: null);

        await sut.LoadAsync("E", "nwtsty", selectedSectionCode: null, reporter);

        Assert.Contains(1.0, reporter.ProgressValues);
        Assert.False(reporter.IsVisible);
    }
    ...
    public void AreAllSectionsFullyCataloged_evaluates_count_and_placeholder_state(
        int sectionId,
        int expectedCount,
        bool expected)
    {
        var method = typeof(SectionListLoader).GetMethod(
            "AreAllSectionsFullyCataloged",
            BindingFlags.NonPublic | BindingFlags.Static);
        Assert.NotNull(method);

        var sections = new SortedDictionary<string, BiblePublicationSection>(StringComparer.OrdinalIgnoreCase)
        {
            ["1"] = new BiblePublicationSection { Id = sectionId, SectionCode = "1", Name = sectionId > 0 ? "Genesis" : "" },
        };

        var actual = (bool)method!.Invoke(null, [sections, expectedCount])!;
        Assert.Equal(expected, actual);
    }

    private sealed class NonEnglishSectionMedia(
        SortedDictionary<string, BiblePublicationSection> sections,
        int expectedCount) : IMediaService
    {
        public int FetchWithProgressCalls { get; private set; }
