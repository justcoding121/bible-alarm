...
public sealed class BiblePublicationTrackServiceBibleAlarmTests
{
    ...
    [Fact]
    public async Task GetTrackAsync_returns_null_when_publication_missing_for_language()
    {
        var (factory, connection) = CreateFactory();
        await using (connection)
        {
            ...
            Assert.Null(await sut.GetTrackAsync("EN", "nwt", "gen", "1"));
        }
    }
