...
namespace Bible.Alarm.ViewModels.Music.TrackSelectionHelpers;
    ...
    IDownloadService downloadService)
{
    ...
    public async Task PopulateTracks(string languageCode, string publicationCode, ObservableCollection<MusicTrackListViewItemModel> tracks)
    {
        ...
        try
        {
            ...
            var tracksFromDb = await Task.Run(async () =>
                await mediaService.GetMusicTracks(languageCode, publicationCode));

            var trackVMs = new ObservableCollection<MusicTrackListViewItemModel>();

            foreach (var track in tracksFromDb.Select(x => x.Value))
            {
                var trackVm = new MusicTrackListViewItemModel(track, logger, urlRefreshService, downloadService);
