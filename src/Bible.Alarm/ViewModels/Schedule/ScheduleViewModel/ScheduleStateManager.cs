...
/// </summary>
public sealed class ScheduleStateManager(ILogger logger)
{
    ...
    public void InitializeStateHandling(IState<ApplicationState> state, Action setBusy, Action setOverlayVisible)
    {
        ...
        {
            MainThread.BeginInvokeOnMainThread(() => OnCurrentScheduleChanged(state));
        }
        else
        {
            _ = Task.Run(async () =>
            {
                ...
                {
                    await MainThread.InvokeOnMainThreadAsync(() => OnCurrentScheduleChanged(state));
                }
            });
        }
    }
