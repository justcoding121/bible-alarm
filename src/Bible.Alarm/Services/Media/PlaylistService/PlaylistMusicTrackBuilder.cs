...
namespace Bible.Alarm.Services.Media.PlaylistService;
...
public class PlaylistMusicTrackBuilder
{
    ...
    private async Task<PlayItem> GetNextMelodyTrackAsync(AlarmSchedule schedule, bool next)
    {
        ...
        var melodyTracks = await melodyMusicService.GetMelodyTracks(melodyMusic.PublicationCode);
    ...
    private async Task<PlayItem> GetNextVocalTrackAsync(AlarmSchedule schedule, bool next)
    {
        ...
        var vocalTracks = await mediaService.GetVocalTracks(vocalMusic.LanguageCode, vocalMusic.PublicationCode);
