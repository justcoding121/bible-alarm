    ...
    : IDisposable
{
    public async Task<Dictionary<string, Language>> GetBibleLanguages()
    {
        ...
        return await dbContext.BibleTranslations.Select(x => x.Language).Distinct()
    ...
    {
        await mediaLookUpService.Verify();
        return await dbContext.BibleTranslations.Where(x => x.Language.Code == languageCode)
            .ToDictionaryAsync(x => x.Code, x => x);
    }
    ...
    public async Task<Dictionary<string, VocalMusic>> GetVocalMusicReleases(string languageCode)
    {
        ...
        return await dbContext.VocalMusic.Where(x => x.Language.Code == languageCode)
            .ToDictionaryAsync(x => x.Code, x => x);
    }
