...
internal static class CollectionViewScrollPositionHelper
{
    ...
    private static int FindItemIndex(IList itemsList, object item)
    {
        for (int i = 0; i < itemsList.Count; i++)
        {
            ...
            else if (item is BibleBookListViewItemModel bookItem &&
                     itemsList[i] is BibleBookListViewItemModel listBookItem &&
                     bookItem.Number == listBookItem.Number)
            {
                return i;
            }
            else if (item is BibleChapterListViewItemModel chapterItem &&
                     itemsList[i] is BibleChapterListViewItemModel listChapterItem &&
                     chapterItem.Number == listChapterItem.Number)
            {
                return i;
            }
        }
