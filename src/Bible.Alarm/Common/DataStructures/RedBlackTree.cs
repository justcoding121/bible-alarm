...
public sealed class RedBlackTree<T> : BstBase<T>, IEnumerable<T> where T : IComparable
{
    ...
    {
        if (enableNodeLookUp) _nodeLookUp = new Dictionary<T, BstNodeBase<T>>(equalityComparer);
    }
    ...
    /// Time complexity: O(n).
    /// </summary>
    public RedBlackTree(IEnumerable<T> sortedKeys, bool enableNodeLookUp = false,
    ...
    public bool HasItem(T value)
    {
        ...
        if (_nodeLookUp != null) return _nodeLookUp.ContainsKey(value);
    ...
    {
        if (_nodeLookUp != null) return (_nodeLookUp[value] as RedBlackTreeNode<T>, Root.Position(value));
    ...
    internal (RedBlackTreeNode<T>, int) InsertAndReturnNode(T value)
    {
        ...
        if (Root == null)
        {
            ...
            if (_nodeLookUp != null) _nodeLookUp[value] = Root;
        ...
        if (_nodeLookUp != null) _nodeLookUp[value] = newNode.Item1;
    ...
    public int Delete(T value)
    {
        ...
        if (_nodeLookUp != null) _nodeLookUp.Remove(value);
    ...
    public T RemoveAt(int index)
    {
        ...
        if (_nodeLookUp != null) _nodeLookUp.Remove(node.Value);
    ...
    private void Delete(RedBlackTreeNode<T> node)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (_nodeLookUp != null) _nodeLookUp[node.Value] = node;
    ...
    internal void Swap(T value1, T value2)
    {
        ...
        if (_nodeLookUp != null)
        {
            _nodeLookUp[node1.Value] = node1;
            _nodeLookUp[node2.Value] = node2;
        }
    }
