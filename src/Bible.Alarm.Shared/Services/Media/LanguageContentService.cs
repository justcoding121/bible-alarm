...
public sealed class LanguageContentService : ILanguageContentService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            else
            {
                ...
                return await FetchPublicationTracksAsync(publicationCode, languageCode, cancellationToken);
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            foreach (var publicationCode in sortedPublications)
            {
                ...
                else if (harvestType == Models.Enums.HarvestType.Flat)
                {
                    ...
                    success = await FetchPublicationTracksAsync(
                        publicationCode, languageCode, cancellationToken);
                }
        ...
        try
        {
            ...
            if (existingPublication != null)
            {
                ...
                if (firstSection != null)
                {
                    ...
                    return await FetchSectionTracksAsync(publicationCode, firstSectionCode, languageCode, cancellationToken);
                }
                else
                {
                    ...
                    var sectionsFetched = await FetchPublicationSectionsAsync(publicationCode, languageCode, cancellationToken);
                    if (sectionsFetched)
                    {
    ...
    /// <summary>
    /// Fetches a single section only (without tracks) and adds it to existing publication or creates new publication.
    /// Note: This currently fetches all sections because FetchPublicationSectionsAsync replaces all sections.
    /// TODO: Optimize to fetch only one section if needed.
    /// </summary>
    private async Task<bool> FetchSingleSectionOnlyAsync(
        MediaDbContext db,
        string publicationCode,
        string sectionCode,
        string normalizedLanguageCode,
        CancellationToken cancellationToken)
    {
        // For now, we fetch all sections. This is acceptable for the initial implementation
        // because FetchPublicationSectionsAsync will create the publication if it doesn't exist,
        // or replace it if it does (which is okay for first-time fetch)
        return await FetchPublicationSectionsAsync(publicationCode, normalizedLanguageCode, cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                logger.Debug("All publications already exist for language {LanguageCode}", languageCode);
                return true;
            }

            logger.Information("Found {Count} missing publications for language {LanguageCode}, fetching...",
                missingPublications.Count, languageCode);
            ...
            foreach (var publicationCode in missingPublications)
            {
                ...
                var harvestType = publicationLanguage.HarvestType ??
                    PublicationTypeHelper.GetHarvestType(publicationCode);

                bool success = false;
                if (harvestType == Models.Enums.HarvestType.Sectioned)
                {
                    // Fetch first section with tracks
                    success = await FetchFirstSectionWithTracksAsync(
                        publicationCode, languageCode, cancellationToken);
                }
                else
                ...
                if (!success)
                {
                    logger.Warning("Failed to fetch publication {PublicationCode} for language {LanguageCode}",
                        publicationCode, languageCode);
                }
            }
