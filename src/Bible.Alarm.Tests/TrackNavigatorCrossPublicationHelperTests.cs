...
public sealed class TrackNavigatorCrossPublicationHelperTests
{
    ...
    private sealed class MusicCategoryStubService : IBiblePublicationService
    {
        public void Dispose()
        {
        }
        ...
        public Task<Dictionary<string, BiblePublication>> GetByLanguageCodeAsync(string languageCode, string? categoryName = null,
            bool filterIsMusicWhenMusicCategory = false, CancellationToken cancellationToken = default) =>
            throw new InvalidOperationException("Not expected for music-category short circuit.");
        ...
        public Task<(string? CategoryCode, bool IsMusic)?> GetPublicationCategoryInfoAsync(string languageCode, string publicationCode,
            CancellationToken cancellationToken = default) =>
            Task.FromResult<(string? CategoryCode, bool IsMusic)?>(("Music", IsMusic: true));
    ...
    private sealed class MissingCategoryStubService : IBiblePublicationService
    {
        public void Dispose()
        {
        }
    ...
    [Fact]
    public async Task TryGetNextAsync_returns_null_when_publication_is_music_category()
    {
        var bible = new MusicCategoryStubService();
        Task<(BiblePublicationSection? Section, BiblePublicationTrack Track)?> Idle(string lang, string pub,
            IFetchProgress? _) =>
            Task.FromResult<(BiblePublicationSection?, BiblePublicationTrack)?>(null);
        ...
        var result = await sut.TryGetNextAsync("E", "singpub", sectionFetchProgress: null);

        Assert.Null(result);
    }

    [Fact]
    public async Task TryGetPreviousAsync_returns_null_when_publication_is_music_category()
    {
        var bible = new MusicCategoryStubService();
        Task<(BiblePublicationSection? Section, BiblePublicationTrack Track)?> Idle(string lang, string pub,
            IFetchProgress? _) =>
        ...
        var result = await sut.TryGetPreviousAsync("E", "singpub", sectionFetchProgress: null);

        Assert.Null(result);
    }

    [Fact]
    public async Task TryGetNextAsync_returns_null_when_publication_category_unknown()
    {
        var bible = new MissingCategoryStubService();
        Task<(BiblePublicationSection? Section, BiblePublicationTrack Track)?> Idle(string lang, string pub,
            IFetchProgress? _) =>
            Task.FromResult<(BiblePublicationSection?, BiblePublicationTrack)?>(null);
        ...
        var result = await sut.TryGetNextAsync("E", "orphan-pub", sectionFetchProgress: null);

        Assert.Null(result);
    }

    [Fact]
    public async Task TryGetPreviousAsync_returns_null_when_publication_category_unknown()
    {
        var bible = new MissingCategoryStubService();
        Task<(BiblePublicationSection? Section, BiblePublicationTrack Track)?> Idle(string lang, string pub,
            IFetchProgress? _) =>
