...
`sonar.coverage.exclusions` drops Android/iOS `Platforms/` sources and XAML code-behind (validated on device, not instrumented on Windows). `Platforms/Windows/**` stays in scope.
