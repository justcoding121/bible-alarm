# Android (Play Store) store listing text

Use the **short** description for the Play Store "Short description" (80 characters max).  
Use the **long** description for the "Full description" (4000 characters max).  
The language list is derived from the media index DB (Bible category, `PublicationLanguages`).

**Full description formatting:** The Play Store does **not** support Markdown (`**bold**`, `-` bullets). Use:
- **Bold:** `<b>text</b>`
- **Bullets:** type the bullet character `•` (U+2022) or use `<ul><li>...</li></ul>`
- Plain line breaks and punctuation are fine.
...
```
Publication audio from jw.org. Alarms, Android Auto. Free, no ads.
```
...
- **Android Auto** support: play and control Bible and music from your car
...
### Play Store–ready (copy into Full description)

Use this block in Play Console. Bold is <b>...</b>; bullets use the • character.
...
Audio is available in <b>over 240 languages</b>, streamed or downloaded from jw.org. At present, only the New World Translation (NWT) is available for Bible reading.
...
## Long description – short variant (if character limit is tight)

If the full description above exceeds the Play Store limit (4000 characters), use this version and omit the full language list. You can keep the line "Bible audio is available in over 240 languages" and optionally add "including Albanian, Amharic, Armenian, … and many more."

<b>Play Store–ready (short variant):</b> Copy the block below into Play Console (HTML + • bullets).
...
Optional: pass a path to `index.zip` or the folder containing `mediaIndex.db`:
