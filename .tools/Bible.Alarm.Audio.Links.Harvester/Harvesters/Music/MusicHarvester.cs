...
internal class MusicHarverster
{
    private static readonly Dictionary<string, string> VocalsPublicationCodeToNameMappings = new(
        ...
        {
            new("osg", "Original Songs"),
            new("sjjc", "\"Sing Out Joyfully\" to Jehovah (2016)"),
            new("snv", "Sing to Jehovah (2014) ")
        });
    ...
    internal static async Task Harvest_Vocal_Music_Links()
    {
        ...
        {
            var harvestLink =
                $"{UrlHelper.JwOrgIndexServiceBaseUrl}?booknum=0&output=json&pub={publication.Key}&fileformat=MP3&alllangs=1&langwritten=E&txtCMSLang=E";
            ...
            foreach (var item in model["languages"])
            {
                ...
                catch (Exception e)
                {
                    ...
                    Console.WriteLine(
                        $"Failed: Harvesting Music track links for {publication.Value} of {language} language. Exception: {e}");
        ...
        foreach (var languagePublication in languageCodeToPublications)
        {
            if (!Directory.Exists($"{DirectoryHelper.IndexDirectory}/media/Music/Vocals/{languagePublication.Key}"))
                Directory.CreateDirectory(
                    $"{DirectoryHelper.IndexDirectory}/media/Music/Vocals/{languagePublication.Key}");
    ...
    private static Dictionary<string, string> melodyPublicationCodeToNameMappings = new(
        new KeyValuePair<string, string>[]
        {
            new("iam", "Sing Praises to Jehovah (1984)")
        });
    ...
    private static async Task<bool> HarvestMusicLinks(string publicationCode, List<string> publicationDownloadCodes,
        string languageCode = null)
    {
        var dir = languageCode == null
        ...
        {
            var harvestLink =
                $"{UrlHelper.JwOrgIndexServiceBaseUrl}?output=json&pub={publicationDownloadCode}&fileformat=MP3&alllangs=0{(languageCode == null ? $"&langwritten=E" : $"&langwritten={languageCode}")}&txtCMSLang=E";
