...
internal class JwBibleHarvester
{
            ...
            ConcurrentDictionary<string, List<string>> languageCodeToEditionsMapping)
    {
        foreach (var publication in biblePublicationCodeToNameMappings)
        {
            ...
            var harvestLink =
                $"{UrlHelper.JwOrgIndexServiceBaseUrl}?booknum=1&output=json&pub={publicationCode}&fileformat=MP3&alllangs=1&langwritten=E&txtCMSLang=E";
    ...
    private static async Task<bool> HarvestBibleLinks(string languageCode, string publicationCode)
    {
        ...
        var harvestLink =
            $"{UrlHelper.JwOrgIndexServiceBaseUrl}?output=json&pub={publicationCode}&fileformat=MP3&alllangs=0&langwritten={languageCode}&txtCMSLang=E";
        ...
        while (bookNumber <= 66)
        {
            ...
            catch (Exception e)
            {
                if (e is JsonReaderException || e is ArgumentException)
                {
                    bookNumber++;
                    harvestLink =
                        $"{UrlHelper.JwOrgIndexServiceBaseUrl}?output=json&pub={publicationCode}&booknum={bookNumber}&fileformat=MP3&alllangs=0&langwritten={languageCode}&txtCMSLang=E";
