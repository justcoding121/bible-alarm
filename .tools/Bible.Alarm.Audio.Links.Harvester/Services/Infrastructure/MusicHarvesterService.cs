    ...
    : IMusicHarvesterService
{
    ...
    private static readonly Dictionary<string, string> MelodyPublicationCodeToNameMappings = new(
        new KeyValuePair<string, string>[]
        {
            new("iam", "Sing Praises to Jehovah (1984)")
        });

    public async Task HarvestVocalMusicLinksAsync()
    {
    ...
    public async Task HarvestMelodyMusicLinksAsync()
    {
    ...
    private async Task<bool> HarvestMusicLinks(string publicationCode, List<string> publicationDownloadCodes,
